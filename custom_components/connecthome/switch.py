...
async def async_setup_entry(
    ...
    entities: list[ConnectHomeSwitch] = []

    for device in coordinator.data.get("devices", []):
        if device.get("type") != DEVICE_TYPE_SWITCH:
            continue
        if IFACE_SWITCH_BINARY not in device.get("interfaces", []):
            continue
        ...
        entities.append(
            ConnectHomeSwitch(
                ...
                device_id=device["id"],
                name=device.get("_display_name", device.get("name", f"Switch {device['id']}")),
                unique_id_suffix=f"switch_{device['id']}",
            )
        )
...
class ConnectHomeSwitch(SwitchEntity):
    ...
    def is_on(self) -> bool:
        ...
        if device is None:
            return False
        return bool(device.get("params", {}).get(PARAM_STATUS, False))
